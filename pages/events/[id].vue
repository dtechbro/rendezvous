...
onMounted(async () => {
  ...
  try {
    ...
    if (response.data?.data?.event) {
      ...
      error.value = 'Event data not found'
    }
  } catch (err) {
  ...
  <div>
    <div class="max-w-7xl mx-auto flex flex-col gap-4 my-5 px-4 sm:px-6 lg:px-8">
      ...
      <div v-if="loading" class="flex justify-center items-center min-h-[50vh]">
        <div class="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#9B51E0]"></div>
      </div>

      <div v-else-if="error" class="flex flex-col items-center justify-center min-h-[50vh] text-center">
        <p class="text-red-500 text-lg mb-4">{{ error }}</p>
        <NuxtLink to="/events" class="text-[#9B51E0] hover:underline">Back to Events</NuxtLink>
      </div>

      <div v-else-if="event" class="space-y-8 pb-10">
        <div class="relative">
          <img 
            :src="event.imageUrl" 
            :alt="event.title"
            class="w-full h-[250px] sm:h-[350px] object-cover rounded-lg"
            loading="lazy"
          />
          ...
          <div class="w-full lg:w-[30%] space-y-8">
            <div class="">
              ...
              <div class="flex gap-4 justify-start">
                <nuxt-link :to="event.organizer.twitterUrl" class="bg-[#783EAD] text-white p-3 rounded-full hover:bg-[#9B51E0] transition-colors">
                  <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84" />
                  </svg>
                </nuxt-link>
                  
                <nuxt-link :to="event.organizer.instagram" class="bg-[#783EAD] text-white p-3 rounded-full hover:bg-[#9B51E0] transition-colors">
                  <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                ...
                <nuxt-link :to="`mailto:${event.organizer.email}`" class="bg-[#783EAD] text-white p-3 rounded-full hover:bg-[#9B51E0] transition-colors">
      ...
      <div v-else class="flex flex-col items-center justify-center min-h-[50vh] text-center">
        <p class="text-gray-600 text-lg mb-4">Event not found</p>
        <NuxtLink to="/events" class="text-[#9B51E0] hover:underline">Back to Events</NuxtLink>
      </div>
    </div>
    <Footer />
